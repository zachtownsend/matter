import { convertStringToCents, formatMoney, sizedImage, getCachedData } from './helpers';
...
describe('Get cached data', () => {
    it('returns empty object if no data source found', () => {
        expect(getCachedData('no-such-path')).toEqual({});
    });

    it('returns json from an existing file', () => {
        expect(getCachedData('test')).toEqual({
            foo: 'bar'
        });
    });
});
